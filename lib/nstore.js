...
function nStore(filename) {
  ...
  function compact() {
    // sys.debug("Compact called:\n" + sys.inspect({compacting: compacting, stale: stale, compactQueue: compactQueue}));
    ...
    // sys.debug(sys.inspect({tmpFile:tmpFile}));
    ...
    keys.forEach(function (key) {
      getByKey(key, function (err, doc, meta) {
        if (err) { throw err; }
        tmpDb.save(meta.key, doc, function (err, meta) {
          if (err) {
            throw err;
          }
          counter--;
  ...
  var timer = setInterval(compact, 10000);
  function destroy() {
    clearInterval(timer);
  ...
  return {
    ...
    // Expose the file descriptor
    fd: fd,
    dbLength: dbLength,

    // Remove all listeners(good for memory cleanup)
    destroy: destroy,
