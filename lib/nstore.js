var utils = require('./nstore/utils'),
...
var nStore = {
  ...
  initialize: function initialize(filename, callback) {
    ...
    this.index = {};
    this.writeQueue = [];
    this.writeQueue.__proto__ = utils.fastArray;
    this.stale = 0;
    this.dbLength = 0;
    this.busy = false;
  ...
  loadDatabase: function (callback) {
    ...
    fs.open(this.filename, 'a+', 0666, function (err, fd) {
      ...
      function emit(line) {
        counter++;
        utils.fsRead(fd, line[0], line[1] - line[0], function (err, key) {
  ...
  compactDatabase: function (clear, callback) {
    ...
    var tmpFile = Path.join(Path.dirname(this.filename), utils.makeUUID(this.index) + ".tmpdb"),
  ...
  get: function getByKey(key, callback) {
    try {
      ...
      utils.fsRead(this.fd, info.position, info.length, function (err, buffer) {
        if (err) { callback(err); return; }
        try {
  ...
  checkQueue: function checkQueue() {
    ...
    Step(
      function writeDocument() {
        utils.fsWrite(self.fd, line, self.dbLength, this);
      },
